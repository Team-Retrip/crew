...
public class NotificationBoardContent {
    ...
    public NotificationBoardContent(String title, String content) {
    ...
    private void validate(String title, String content) {
        if (title.length() > TITLE_LENGTH_LIMIT) {
            throw new IllegalArgumentException("공지 게시글 제목은 " + TITLE_LENGTH_LIMIT + "자를 넘을 수 없습니다.");
        }
        if (content.length() > CONTENT_LENGTH_LIMIT) {
            throw new IllegalArgumentException("공지 게시글 내용은 " + CONTENT_LENGTH_LIMIT + "자를 넘을 수 없습니다.");
        }
    }
}
